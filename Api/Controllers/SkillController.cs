...
namespace Api.Controllers
{
    public class SkillController : BaseController<Skill>
    {
       ...
       // [Auth(AuthActionTypes.Read, AuthRoles.Administrator, AuthRoles.Finance, AuthRoles.OrganizationUnit)]
        ...
        public override IQueryable<Skill> Get()
        {
       ...
       // [Auth(AuthActionTypes.Read, AuthRoles.Administrator, AuthRoles.Finance, AuthRoles.OrganizationUnit)]
        ...
        public override SingleResult<Skill> Get([FromODataUri] Guid key)
        {
            return SingleResult.Create(Context.Set<Skill>().Where(e => e.Id == key));
        }

        [HttpPost]
        //[Auth(AuthActionTypes.Create, AuthRoles.Administrator)]
        public override async Task<IHttpActionResult> Post(Skill entity)
        {
            // TODO: Error handling
       ...
       // [Auth(AuthActionTypes.Update, AuthRoles.Administrator)]
        public override async Task<IHttpActionResult> Put([FromODataUri] Guid key, Skill entity)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
